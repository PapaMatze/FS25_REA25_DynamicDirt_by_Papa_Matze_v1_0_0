-- Compatibility and LS25 upgrade header
-- Converted to be LS25-ready (private + public release) by Papa_Matze
-- Adds a compatibility shim and diagnostic logging for LS25.
if g_currentMission == nil then
    print("[READynamicDirt] Warning: g_currentMission is nil. This mod expects to run inside Farming Simulator.")
else
    if g_currentMission.missionInfo ~= nil and g_currentMission.missionInfo.gameVersion ~= nil then
        -- attempt to detect game major version if available
        local gv = tostring(g_currentMission.missionInfo.gameVersion)
        print(string.format("[READynamicDirt] Detected gameVersion: %s", gv))
    else
        -- fallback: print current mission info available
        print("[READynamicDirt] Running, unable to read structured gameVersion from g_currentMission.missionInfo")
    end
end
...
print("----- REA25 Dynamic Dirt by Papa_Matze v1.0.0 -----")
print("----- REA25 Dynamic Dirt by Papa_Matze v1.0.0 -----")
print("----- REA25 Dynamic Dirt by Papa_Matze v1.0.0 -----")
READirt = {};
...
-- Realistic dirt scaling
if g_currentMission ~= nil and g_currentMission.vehicleDirtManager ~= nil then
    g_currentMission.vehicleDirtManager.dirtScale = 0.65
    g_currentMission.vehicleDirtManager.wheelDirtScale = 3.2
    g_currentMission.vehicleDirtManager.underbodyDirtScale = 2.2
end
...
function READirt:GetIsLowSpots(wX,wY,wZ,SearchSquare,MinWaterDepth,MaxWaterDepth,SearchLowSpotSize)
	...
	if PointIsALowSpot then
		...
		if LowSpot.DebugActive then
			print("----- REA25 Dynamic Dirt by Papa_Matze v1.0.0 -----");
			print("Low spot created by REA25 Dynamic Dirt");
...
if READirt.ModActivated == nil then
	...
	print("REA25 Dynamic Dirt mod activated")
